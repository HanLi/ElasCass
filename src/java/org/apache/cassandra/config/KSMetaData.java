...
import org.apache.cassandra.locator.SimpleReplicationStrategy;
...
public final class KSMetaData
{
    ...
    private KSMetaData(String name, Class<? extends AbstractReplicationStrategy> strategyClass, Map<String, String> strategyOptions, boolean durableWrites, Iterable<CFMetaData> cfDefs)
    {
        ...
        if(strategyClass==null || strategyClass==NetworkTopologyStrategy.class)
        {
        	this.strategyClass = SimpleReplicationStrategy.class;
        }
        else
        {
        ...
        if(this.strategyClass.equals(SimpleReplicationStrategy.class))
        {
        	if( this.strategyOptions.get("replication_factor")==null )
        		this.strategyOptions.put("replication_factor", DatabaseDescriptor.getReplicationFactor().toString());
        }
    }
    ...
    private static void maybeAddReplicationFactor(Map<String, String> options, String cls, Integer rf)
    {
        if (rf != null && (cls.endsWith("SimpleStrategy") 
        		...
        		|| cls.endsWith("SimpleReplicationStrategy")))
    ...
    public static KSMetaData fromAvro(org.apache.cassandra.db.migration.avro.KsDef ks)
    {
        ...
        if (ks.strategy_options != null)
        {
            for (Map.Entry<CharSequence, CharSequence> e : ks.strategy_options.entrySet())
            {
                ...
                if (repStratClass == SimpleReplicationStrategy.class && name.trim().toLowerCase().equals("replication_factor"))
                    continue;
