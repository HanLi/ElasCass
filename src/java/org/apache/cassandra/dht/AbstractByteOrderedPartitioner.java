...
public abstract class AbstractByteOrderedPartitioner implements IPartitioner<BytesToken>
{
    ...
    private final Token.TokenFactory<byte[]> tokenFactory = new Token.TokenFactory<byte[]>() 
    {
		...
		@Override
		public ByteBuffer toByteArray(Collection<Token<byte[]>> tokens) 
		{
			int size = 0;
			for(Token<byte[]> bytesToken : tokens)
			{
				size += 4 + bytesToken.token.length;
			}
			
			ByteBuffer byteBuffer = ByteBuffer.allocate(size);
			for(Token<byte[]> bytesToken : tokens)
			{
				byteBuffer.putInt(bytesToken.token.length);
				byteBuffer.put(bytesToken.token);
			}
		...
		@Override
		public Collection<Token<byte[]>> fromByteArrayToCollection(ByteBuffer bytes) 
		{
			Set<Token<byte[]>> tokens = new HashSet<Token<byte[]>>();
			while(bytes.remaining()>0)
			{
				int length = bytes.getInt();
				byte[] array = new byte[length];
				bytes.get(array);
				tokens.add(new BytesToken(array));
			}
			return tokens;
		}

		@Override
		public String toString(Collection<Token<byte[]>> tokens) 
		{
			StringBuilder sb = new StringBuilder();
			for(Token<byte[]> bytesToken : tokens)
			{
				sb.append(Hex.bytesToHex(bytesToken.token));
		...
		@Override
		public Collection<Token<byte[]>> fromStringToCollection(String string) 
		{
			String[] strs = string.split(TokenSerializer.DELIMITER_TOKEN_STR, -1);
			Set<Token<byte[]>> set = new HashSet<Token<byte[]>>();
			for(String str : strs) if(str.length() > 0)
			{
				set.add(new BytesToken(Hex.hexToBytes(str)));
			}
			return set;
		}
    };
