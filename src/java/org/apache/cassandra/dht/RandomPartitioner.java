...
public class RandomPartitioner implements IPartitioner<BigIntegerToken>
{
    ...
    private final Token.TokenFactory<BigInteger> tokenFactory = new Token.TokenFactory<BigInteger>() 
    {
		...
		@Override
		public ByteBuffer toByteArray(Collection<Token<BigInteger>> tokens) 
		{
			int size = 0;
			for(Token<BigInteger> bigIntegerToken : tokens)
			{
				size += 4 + bigIntegerToken.token.toByteArray().length;
			}
			
			ByteBuffer byteBuffer = ByteBuffer.allocate(size);
			for(Token<BigInteger> bigIntegerToken : tokens)
			{
				byteBuffer.putInt(bigIntegerToken.token.toByteArray().length);
				byteBuffer.put(bigIntegerToken.token.toByteArray());
			}
		...
		@Override
		public Collection<Token<BigInteger>> fromByteArrayToCollection(ByteBuffer bytes) 
		{
			Set<Token<BigInteger>> tokens = new HashSet<Token<BigInteger>>();
			while(bytes.remaining()>0)
			{
				...
				Token<BigInteger> bigIntegerToken = new BigIntegerToken(new BigInteger(array));
				tokens.add(bigIntegerToken);
			}
			return tokens;
		}

		@Override
		public String toString(Collection<Token<BigInteger>> tokens) 
		{
			StringBuilder sb = new StringBuilder();
			for(Token<BigInteger> bigIntegerToken : tokens)
			{
				sb.append(bigIntegerToken.token.toString());
		...
		@Override
		public Collection<Token<BigInteger>> fromStringToCollection(String string) 
		{
			String[] strs = string.split(TokenSerializer.DELIMITER_TOKEN_STR, -1);
			Set<Token<BigInteger>> set = new HashSet<Token<BigInteger>>();
			for(String str : strs) if(str.length() > 0)
			{
				set.add(new BigIntegerToken(new BigInteger(str)));
			}
			return set;
		}
    };
