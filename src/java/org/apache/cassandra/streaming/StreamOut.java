...
public class StreamOut
{
    ...
    */
    public static void transferRanges(InetAddress target, ColumnFamilyStore cfs, Collection<Range> ranges, Runnable callback, OperationType type)
    {
        StreamOutSession session = StreamOutSession.create(target, callback);
        transferRanges(session, cfs, ranges, type);
    }
     ...
     */
    private static void flushSSTables(ColumnFamilyStore cfstore) throws IOException
    {
        logger.info("Flushing memtables for {}...", cfstore);
    ...
    */
    public static void transferRanges(StreamOutSession session, ColumnFamilyStore cfs, Collection<Range> ranges, OperationType type)
    {
        ...
        try
        {
            ...
            for (Range range : ranges)
                sstables = Iterables.concat(sstables, cfs.markCurrentSSTablesReferenced(range));
            transferSSTables(session, sstables, type);
        }
        catch (IOException e)
        {
            throw new IOError(e);
     ...
     */
    public static void transferSSTables(StreamOutSession session, Iterable<SSTableReader> sstables, OperationType type) throws IOException
    {
        List<PendingFile> pending = createPendingFiles(sstables, type);
    ...
    private static List<PendingFile> createPendingFiles(Iterable<SSTableReader> sstables, OperationType type)
    {
        List<PendingFile> pending = new ArrayList<PendingFile>();
        for (SSTableReader sstable : sstables)
        {
            Descriptor desc = sstable.descriptor;
            Map<String, Long> components = SSTableReader.getComponentsWithLengthFor(desc);
            ...
            pending.add(new PendingFile(sstable, desc, components, type,
