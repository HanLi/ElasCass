...
public class StreamIn
{
    ...
    public static void requestRanges(InetAddress source, String tableName, String cfName, Collection<Range> ranges, Runnable callback, OperationType type)
    {
        requestRanges(source, tableName, Table.open(tableName).getColumnFamilyStore(cfName), ranges, callback, type);
    }

    /**
     * Request ranges to be transferred from specific CFs
     */
    public static void requestRanges(InetAddress source, String tableName, ColumnFamilyStore columnFamily, Collection<Range> ranges, Runnable callback, OperationType type)
    {
        ...
        try {
			FileUtils.createDirectory(DatabaseDescriptor.getAllDataFileLocationsForTable(tableName)[0] + 
					File.separator + source.getHostAddress());
		}
        catch (IOException e) {}
    ...
    public static PendingFile getContextMapping(PendingFile remote) throws IOException
    {
        ...
        if (!remotedesc.isStreamCompatible())
            throw new UnsupportedOperationException(String.format("SSTable %s is not compatible with current version %s",
            		remotedesc.toString(), Descriptor.CURRENT_VERSION));
