...
public class StreamInSession
{
    ...
    public void addFiles(Collection<PendingFile> files)
    {
        for (PendingFile file : files)
        {
            if(logger.isDebugEnabled())
                logger.debug("Adding file {} to Stream Request queue", file.desc.toString());
            this.files.add(file);
        }
    }
    ...
    public void finished(PendingFile remoteFile, PendingFile localFile) throws IOException
    {
        ...
        SSTableReader sstable = ColumnFamilyStore.loadNewSSTable(localFile.desc, localFile.components.keySet());
        if(sstable!=null && sstable.getRange()!=null)
        {
        	StorageService.instance.addSSTable(localFile.desc.ksname, localFile.desc.cfname, sstable.getRange());
        }
        
        files.remove(remoteFile);
        if (remoteFile.equals(current))
            current = null;
        StreamReply reply = new StreamReply(remoteFile.desc.toString(), getSessionId(), StreamReply.Status.FILE_FINISHED);
    ...
    {
        StreamReply reply = new StreamReply(remoteFile.desc.toString(), getSessionId(), StreamReply.Status.FILE_RETRY);
