...
class StreamRequestMessage implements MessageProducer
{
    ...
    // if this is specified, ranges and CF should not be.
    protected final PendingFile file;
    
    // if these are specified, file shoud not be.
    protected final Collection<Range> ranges;
    protected final ColumnFamilyStore columnFamily;
    protected final OperationType type;

    StreamRequestMessage(InetAddress target, Collection<Range> ranges, ColumnFamilyStore columnFamily, long sessionId, OperationType type)
    {
        this.target = target;
        this.ranges = ranges;
    ...
    public String toString()
    {
        ...
        if (file == null)
        {
            sb.append(columnFamily.table.name);
            sb.append("@");
            sb.append(columnFamily.columnFamily);
            sb.append("@");
            sb.append(target);
            sb.append("------->");
    ...
    private static class StreamRequestMessageSerializer implements IVersionedSerializer<StreamRequestMessage>
    {
        public void serialize(StreamRequestMessage srm, DataOutput dos, int version) throws IOException
        {
            ...
            else
            {
                ...
                dos.writeUTF(srm.columnFamily.table.name);
                dos.writeUTF(srm.columnFamily.columnFamily);
                ...
                {
                	dos.writeInt(srm.columnFamily.metadata.cfId);
                }
            }
        }
        ...
        public StreamRequestMessage deserialize(DataInput dis, int version) throws IOException
        {
            ...
            else
            {
                ...
                return new StreamRequestMessage(target, ranges, cfs, sessionId, type);
            }
        }
