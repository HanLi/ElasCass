...
public class RangeCompactionTask extends CompactionTask
{
	protected static final Logger logger = LoggerFactory.getLogger(RangeCompactionTask.class);
    protected final Range range;
    
    protected static final long maxSSTableSize = DatabaseDescriptor.getMaximumSSTableSize();
    protected static final AtomicInteger concurrentTasks = new AtomicInteger(0);
    
    public RangeCompactionTask(ColumnFamilyStore cfs, Collection<SSTableReader> sstables, 
    		final int gcBefore, Range range)
    {
    ...
    private int executeInternal(CompactionManager.CompactionExecutorStatsCollector collector) throws IOException
    {
        ...
        try
        {
            ...
            SSTableWriter writer = cfs.createCompactionWriter(
            		range, keysPerSSTable, compactionFileLocation, toCompact);
            ...
            while (nni.hasNext())
            {
                ...
                if (!nni.hasNext() || newSSTableSegmentThresholdReached(writer, position))
                {
                    ...
                    if (nni.hasNext())
                    {
                        writer = cfs.createCompactionWriter(range, keysPerSSTable, compactionFileLocation, toCompact);
                        writers.add(writer);
    ...
    /**
     * @return true if the proposed compaction is worth proceeding with. 
     */
    @Override
    protected boolean isCompactionInteresting(Set<SSTableReader> toCompact)
    {
        if (isUserDefined || toCompact.size() >= 2)
            return true;
        logger.info(String.format("Nothing to compact in %s.  " +
    ...
    @Override
    public RangeCompactionTask compactionFileLocation(String compactionFileLocation)
    {
    ...
    @Override
    public RangeCompactionTask isUserDefined(boolean isUserDefined)
    {
    ...
    {
        return position > maxSSTableSize;
    }
    
    public static int getConcurrentTaskCount()
    {
