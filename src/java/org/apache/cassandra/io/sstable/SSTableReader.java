...
public class SSTableReader extends SSTable
{
    ...
    */
    ...
    {
    	return sstableMetadata.range;
    }
    
    public static Range validateAndGetRange(Iterable<SSTableReader> sstables)
    {
    	...
    	Range range = sstable.getRange();
    	if(range == null)
    		throw new IllegalArgumentException("The range for SSTable " + sstable + " should not be null.");
    	
    	while(iter.hasNext())
    	{
    		sstable = iter.next();
    		Range rng = sstable.getRange();
    		
    		if(rng == null)
        		throw new IllegalArgumentException("The range for SSTable " + sstable + " should not be null.");
    		if(!range.equals(rng))
    			throw new IllegalArgumentException("The range for all the SSTables should be the same.");
    	}
