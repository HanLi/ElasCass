...
//    			null);
    	throw new UnsupportedOperationException(
    			"The range is needed for initiating an SSTableMetadata object");
    }

    private SSTableMetadata(Range range, EstimatedHistogram rowSizes, EstimatedHistogram columnCounts, ReplayPosition replayPosition, long maxTimestamp, double cr, String partitioner)
    {
    	this.range = range;
    ...
    {
    	Token token = StorageService.getPartitioner().getToken(SystemTable.TOKEN);
    	return new Range(token, token);
    }
    ...
    public static class Collector
    {
        ...
        {
            return new SSTableMetadata(range,
                    				   estimatedRowSize,
    ...
    public static class SSTableMetadataSerializer
    {
        ...
        public void serialize(SSTableMetadata sstableStats, DataOutput dos) throws IOException
        {
            ...
            Token.serializer().serialize(sstableStats.range.left, dos);
            Token.serializer().serialize(sstableStats.range.right, dos);
        ...
        {
        	Token left = Token.serializer().deserialize(dis);
        	Token right = Token.serializer().deserialize(dis);
            ...
            return new SSTableMetadata(range, rowSizes, columnCounts, replayPosition, maxTimestamp, compressionRatio, partitioner);
        }
    }
}
