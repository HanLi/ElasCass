...
public class StorageProxy implements StorageProxyMBean
{
    ...
    throws UnavailableException, TimeoutException, IOException
    {
        ...
        Integer cfId = mutation.getColumnFamily().id();
        AbstractReplicationStrategy rs = Table.open(table).getColumnFamilyStore(cfId).getReplicationStrategy();

        Collection<InetAddress> writeEndpoints = rs.getWriteEndpoints(mutation.key());
...
//    private static Collection<InetAddress> getWriteEndpoints(Integer cfId, ByteBuffer key)
//    {
//        StorageService ss = StorageService.instance;
//        List<InetAddress> naturalEndpoints = ss.getNaturalEndpoints(cfId, key);
//        return ss.getTokenMetadata(cfId).getWriteEndpoints(StorageService.getPartitioner().getToken(key), naturalEndpoints);
//    }
    ...
    throws IOException
    {
        ...
        Runnable runnable = new WrappedRunnable()
        {
            public void runMayThrow() throws IOException
            {
                ...
                try
                {
                	Integer cfId = mutation.getColumnFamilyId();
                	TokenMetadata tm = StorageService.instance.getTokenMetadata(cfId);
                    TokenFactory tf = StorageService.getPartitioner().getTokenFactory();
                	
                	Set<Token> tokens = Sets.newHashSet(tm.getTokens(target));
                    for(Token token : tokens)
                    {
                    	ByteBuffer tokenbytes = tf.toByteArray(token);
    ...
    {
    	CFMetaData cfmd = cm.getColumnFamily().metadata();
        InetAddress endpoint = findSuitableEndpoint(cfmd, cm.key(), localDataCenter);
        ...
        else
        {
            ...
            AbstractReplicationStrategy rs = Table.open(table).getColumnFamilyStore(cfmd.cfId).getReplicationStrategy();
            Collection<InetAddress> writeEndpoints = rs.getWriteEndpoints(cm.key());
     ...
     */
    private static InetAddress findSuitableEndpoint(CFMetaData cfmd, ByteBuffer key, String localDataCenter) throws UnavailableException
    {
        IEndpointSnitch snitch = DatabaseDescriptor.getEndpointSnitch();
        List<InetAddress> endpoints = StorageService.instance.getLiveNaturalEndpoints(cfmd.ksName, cfmd.cfName, key);
    ...
    */
    ...
    throws IOException, UnavailableException, TimeoutException
    {
        ...
        try
        {
            ...
            List<AbstractBounds> ranges = getRestrictedRanges(command.keyspace, command.column_family, command.range);
            for (AbstractBounds range : ranges)
            {
                List<InetAddress> liveEndpoints = StorageService.instance.getLiveNaturalEndpoints(
                		command.keyspace, command.column_family, range.right);
     ...
     * @param keyspace 
     */
    static List<AbstractBounds> getRestrictedRanges(String keyspace, String columnfamily, final AbstractBounds queryRange)
    {
        ...
        TokenMetadata tokenMetadata = StorageService.instance.getTokenMetadata(keyspace, columnfamily);

        List<AbstractBounds> ranges = new ArrayList<AbstractBounds>();
        // divide the queryRange into pieces delimited by the ring and minimum tokens
        Iterator<Token> ringIter = TokenMetadata.ringIterator(tokenMetadata.getSortedTokens(), queryRange.left, true);
    ...
    public static List<Row> scan(final String keyspace, final String column_family, IndexClause index_clause, SlicePredicate column_predicate, ConsistencyLevel consistency_level)
    throws IOException, TimeoutException, UnavailableException
    {
        IPartitioner p = StorageService.getPartitioner();

        Token leftToken = index_clause.start_key == null ? p.getMinimumToken() : p.getToken(index_clause.start_key);
        List<AbstractBounds> ranges = getRestrictedRanges(keyspace, column_family, new Bounds(leftToken, p.getMinimumToken()));
        ...
        for (AbstractBounds range : ranges)
        {
            List<InetAddress> liveEndpoints = StorageService.instance.getLiveNaturalEndpoints(keyspace, column_family, range.right);
            DatabaseDescriptor.getEndpointSnitch().sortByProximity(FBUtilities.getBroadcastAddress(), liveEndpoints);
