...
public class WriteResponseHandler extends AbstractWriteResponseHandler
{
    ...
    protected WriteResponseHandler(Collection<InetAddress> writeEndpoints, ConsistencyLevel consistencyLevel, String table, String cfName)
    {
        super(writeEndpoints, consistencyLevel);
        responses = new AtomicInteger(determineBlockFor(table, cfName));
    }
    ...
    public static IWriteResponseHandler create(Collection<InetAddress> writeEndpoints, ConsistencyLevel consistencyLevel, String table, String cfName)
    {
        return new WriteResponseHandler(writeEndpoints, consistencyLevel, table, cfName);
    }
    ...
    protected int determineBlockFor(String table, String cfName)
    {
        switch (consistencyLevel)
        {
            ...
            case THREE:
                return 3;
            case QUORUM:
                return (Table.open(table).getColumnFamilyStore(cfName).getReplicationStrategy().getReplicationFactor() / 2) + 1;
            case ALL:
                return Table.open(table).getColumnFamilyStore(cfName).getReplicationStrategy().getReplicationFactor();
            default:
